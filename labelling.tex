...
\snote{Piotrek: we need a comment on $\log\log h$ below; so $m,h \leq n$, right?}
...
$|\alpha(y)|=\log n - \log|T_y| + \Oh(\log\log h)$ for each $y\in\{1,\ldots,h\}$, and $A(\alpha(i),\alpha(j))$ outputs $0$, $1$, $-1$, or $\perp$, depending on the value of $j$ to be $i$, $i+1$, $i-1$, or some other value, respectively.
...
First note that we can decode each block of the label just because $\alpha(\cdot)$ and $\gamma(\cdot)$ are prefix-free. \snote{Piotrek: Hmm. Not really.}
