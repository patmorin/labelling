...
\begin{lem}\lemlabel{t-tree-labelling}
  Let $H$, $S$, $X$, $Q$, and $T$ be defined as above.  Then $|\tau_{T}(v)|=h(T) + O(t(\log t + \log h(T)))$ for every $v\in Q$  and there exists a function $\mathds{T}:(\{0,1\}^*)^2\to \Z\cup\{\perp\}$ such that  for every $v,w\in Q$, 
  \[
...
\begin{enumerate}
  ...
  \item $y_1=y_2$.  Let $y=y_1=y_2$.  In this case (PC2)--(PC4) contains $\tau_{T_y}(v_1)$ and $\tau_{T_y}(v_2)$ (defined before \lemref{t-tree-labelling}) and we use this to test if $v_1v_2\in E(H)$.  If not, then $v_1v_2\not\in E(H\boxtimes P)$ so $z_1z_2\not\in E(G)$.  
