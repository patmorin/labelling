...
\begin{proof}
  ...
  The code $\alpha(i)$ for $i$ consists of three parts.  The first part, $\gamma(|\sigma_T(i)|)$, is the Elias encoding of the length of the path $P_T(i)$ from the root to $i$ in $T$. The second part $\sigma_T(i)$ encodes the left/right turns along this path. The third part, $\delta_T(i)$, is defined after \obsref{predecessor-encoding}. 
...
A naive approach to create labels for each $H_i$ is to use a labelling scheme for bounded treewidth graphs; roughly, this entails building a specific balanced binary search tree $T_i$ on the vertices of $H_i$ in such a way that the label of each vertex $(v, i)$ is of length $\log |T_i|$ plus a lower-order term. The label of $(v, i)$ encodes the position of $v$ in $T_i$ plus some small extra information (see~\secref{t-tree-labelling}).
...
The bulk insertion operation, in which a finite set $I\subset  \R\setminus V(T)$ of new values are inserted into a binary search tree $T$, is implemented as follows: Let $z_0,\ldots,z_{|T|}$ denote the external nodes of $T$.  For each $i\in\{0,\ldots,|T|\}$, let $I_i$ consist of all $x\in I$ such that $P_T(x)$ ends at $z_i$. 
...
\begin{proof}
  That $T'$ is a supergraph of $T$ is obvious.  That $h(T')\le h(T)+2$ comes from the fact that, for each $i\in\{0,\ldots,|T|\}$, $|T_i|=|I_i|\le 3$ and $T_i$ is perfectly balanced, so $h(T_i)\le\lfloor\log 3\rfloor = 1$. 
  Note that $|I_i|\le 3$ since $V(T)$ $3$-chunks $I$.  
  Any root-to-leaf path in $T'$ consists of a root-to-leaf path in $T$ followed by at most 2 elements of $T_i$ for some $i\in\{1,\ldots,|T|\}$.  Therefore the length of any root-to-leaf path in $T'$ is at most $h(T)+2$.
\end{proof}
...
\begin{proof}
   Since $V(T)\setminus D$ $3$-chunks $D$, we have $|D|\le 3(|V(T)|-|D| + 1) \le 6(|V(T)|-|D|)$, so $|D|\le (6/7)|V(T)|$. 
\end{proof}

\subsection{Rebalancing}

%At the beginning of a round, before bulk insertion and bulk deletion, a rebalancing operation is performed.  
...
%Finally, recall that a bulk tree works in phases and, within a phase, maintains a counter $y$ that counts the rounds of the phase from $0$ to $y^*$.  The $\textsc{BulkBalance}(\theta,k)$ operation executes once, at the beginning of each round, with the argument $\theta=y(k+1)$.

\subsection{Bulk Tree Sequences}

\snote{PM \& GJ: THIS SECTION IS CURRENTLY BEING EDITED}


%\subsection{Analysis of Height}

In this section we show that a bulk tree always has asymptotically optimal height.

\begin{lem}\lemlabel{bulktree-old-B-properties}
Let $T$ be a bulk tree, let $T_0,\ldots,T_{y^*}$ be the states of $T$ so that $T_y$ is the state before the $y$-th round. 
Then, for each $y\in\{0,\ldots,y^*\}$
\begin{compactenum}[(i)]
\item $h(T_y)\le h(T_0) + 3y$;\label{lemma-item-B1}
\item each subtree of $T_y$ rooted at a node of depth $y(k+1)$ has size at most $|T_0|\cdot\left(7/2^k\right)^{y}$.\label{lemma-item-B2}
\end{compactenum}
\end{lem}
\begin{proof}
The proof is by induction on $y$.  For the base case $y=0$, both properties are trivial: \itemref{lemma-item-B1} asserts that $h(T_0)\le h(T_0)$ and \itemref{lemma-item-B2} asserts that the subtree of $T_0$ rooted at the root of $T_0$ has size at most $|T_0|$.
  ...
  Lemmas~\ref{lem:balance-depth} (rebalancing), \ref{lem:insertion-depth} (insertions), and \ref{lem:deletion-signature} (deletions) imply that $h(T_{y+1})\le h(T_{y})+3 \leq h(T_0) + 3y +3 \leq h(T_0)+3(y+1)$. Thus, \itemref{lemma-item-B1} holds for $T_{y+1}$.
  
  Next we establish \itemref{lemma-item-B2}. Assume that \itemref{lemma-item-B2} holds for $T_{y}$. Thus, every subtree of $T_{y}$ rooted at a node of depth $y(k+1)$ has size at most $|T_{0}|\cdot(7/2^k)^{y}$.  The first step in round $y$ is to execute $\textsc{BulkBalance}(y(k+1))$.  By \lemref{balance-weight}, this results in a tree $T_{y}'$ in which every subtree rooted at a node of depth $(y+1)(k+1)$ has size at most $\tfrac{|T_{0}|(7/2^k)^{y}}{2^k}$.  The second step in round $y$ is to perform a bulk insertion on $T_{y}'$ to obtain a new tree $T_{y-1}''$.  
  ...
  By \lemref{insertion-size}, every subtree of $T_{y}''$ rooted at a node of depth $(y+1)(k+1)$ has size at most $7\cdot\tfrac{|T_{0}|(7/2^k)^{y}}{2^k}$.  Finally, the third step in round $y$ is to perform a bulk deletion on $T_{y}''$ to obtain $T_{y+1}$.  Bulk deletion does not increase the size of any subtree, so every subtree of $T_{y+1}$ rooted at a node of depth $(y+1)(k+1)$ has size at most $|T_{0}|\cdot(7/2^k)^{y+1}$, as required.
\end{proof}

\begin{cor}\corlabel{height-y-star}
Let $T$ be a bulk tree and let $T_{y^*}$ be the state of $T$ before the $y^*$-th round. 
Then,
\[
h(T_{y^*}) \leq y^*(k+1).
\]
\end{cor}
\begin{proof}
Note that when we take the logarithm of the upper bound in \lemref{bulktree-old-B-properties}\itemref{lemma-item-B2} for $y=y^*$, and we plug the definition of $y^*$, we have
\[
\log|T_0| - y^*\cdot\log\left(2^k/7\right) 
\leq \log|T_0|-\tfrac{\log|T_0|}{\log\left(2^k/7\right)}\cdot\log\left(2^k/7\right)
=0.
\]
Thus,
each subtree of $T_{y^*}$ rooted at a node of depth $y^*(k+1)$ has size at most 
$|T_0|\cdot\left(7/2^k\right)^{y^*} \leq 1$.
\end{proof}

\begin{lem}\lemlabel{bulktree-height-i}
  Let $T$ be a bulk tree, let $T_0,\ldots,T_{y^*}$ be the states of $T$ so that $T_y$ is the state before the $y$-th round for each $y\in\{0,\ldots,y^*\}$, and let $r_0=h(T_0)-\log|T_0|$.  Then,
  \begin{compactenum}[(i)]
    \item $h(T_y)= \log|T_y| + \Oh(k^{-1}\log|T_y|) + r_0$, for each $y\in\{0,\ldots,y^*\}$;
    \label{bulktree-height-item-i}
    \item $h(T_{y^*}) = \log|T_{y^*}|+\Oh(k+k^{-1}\log|T_{y^*}|)$.\label{bulktree-height-item-ii}
  \end{compactenum}
\end{lem}
\begin{proof}
Basically the two items follow from the corresponding two items of \lemref{bulktree-old-B-properties}.
Let $I_0,\ldots,I_{y^*-1}$ and $D_0,\ldots,D_{y^*-1}$ be the sets inserted and deleted so that $I_y$ and $D_y$ are the bulk insertions and bulk deletions performed in round $y$, for each $y\in\{0,\ldots,y^*-1\}$.  First, recall that $|D_y|\le (6/7)|T_y|$, which implies that $|T_{y+1}|\ge |T_y|-|D_y|\ge |T_y|/7$.  Iterating this beginning with $T_0$ implies that 
  \begin{equation}
    |T_0|/7^y\le |T_y|\quad \text{and} \quad \log|T_0| \le \log|T_y| + y\log 7, \eqlabel{height-diff}
  \end{equation}
  for each $y\in\{0,\ldots,y^*\}$. 

Now observe that, for each $y\in\{0,\ldots,y^*\}$,  we have
\begin{align}
y^*
&\le \tfrac{\log |T_0|}{k-\log 7}+1\le \tfrac{\log |T_y|+y^*\log 7}{k-\log 7}+1,
&\text{(by \eqref{y-star-def}, \eqref{height-diff}, and since $y\leq y^*$)}\nonumber\\
\intertext{and rewriting this yields}
y^*
&\le \tfrac{\log |T_y|}{k-2\log 7}+ \tfrac{k-\log 7}{k-2\log 7} = \Oh(k^{-1}\log|T_y|).
\eqlabel{ystar-bound}
\intertext{The first item of the lemma follows as for each $y\in\{0,\ldots,y^*\}$, we have}
h(T_y) 
& \le h(T_0) + 3y \nonumber
...
& = \left(\tfrac{k+1}{k-\log 7}\right)\cdot\log|T_0| + (k+1)\nonumber\\
& \le \left(\tfrac{k+1}{k-\log 7}\right)\cdot(\log|T_{y^*}| + y^*\log 7) + (k+1)
&\text{(by \eqref{height-diff})}\nonumber\\
& = \log|T_{y^*}| + \Oh(k+k^{-1}\log |T_{y^*}|).
&\text{(by \eqref{ystar-bound})}\nonumber
\end{align}
\end{proof}
...
\begin{lem}\lemlabel{bulk-tree-height}
  For each bulk tree sequence $T_1,\ldots,T_h$ and each $y\in\{1,\ldots,h\}$, we have
\[
...
\begin{proof}
  ...
  \end{enumerate}
  To verify this we have to dig into the details of the rebalancing operations, which involves a single call to $\textsc{BulkBalance}(\theta,k)$ which calls $\textsc{Balance}(x,k)$ on each of the nodes $x$ in $T_y$ of depth $\theta$.    
  For each node $z$ in $T_y$, a call to $\textsc{Balance}(x,k)$ does not affect $\sigma_{T_y}(z)$ unless $x$ is a $T_y$-ancestor of $z$.  Thus, for each $z$ in $T_y$, we can focus on the changes to $\sigma_{T_y}(z)$ caused by the at most one call to $\textsc{Balance}(x,k)$ where $x$ is an ancestor of $z$. Note that, if $d_{T_y}(z)<\theta$, then $\sigma_{T_y}(z)=\sigma_{T_y'}(z)$, in which case $\nu_{y}(z)$ has $\Oh(1)$ bits indicating that this is the case and parts ($\nu_1(z)$)--($\nu_4(z)$) of $\nu_y(z)$ are empty.
  
  Otherwise, $z$ has some depth-$\theta$ $T_y$-ancestor $x$ on which $\textsc{Balance}(x,k)$ executes. In this case $\nu_{1}(z)=\theta$, indicating that $b$ should be split into $b^-$ and $b'$ at position $\theta$.  
  The $\textsc{Balance}(x,k)$ operation first identifies two sets of nodes $Z$ and $X$ that will eventually form the perfectly balanced tree $\hat{T}_0$ to which the subtrees $A_0,\ldots,A_p$ are attached.  For each node $z\in Z\cup X$, $\nu_{2}(z):=|b'|$ and $\nu_{4}(z):=\sigma_{\hat{T}_0}(z)$ indicating that all of $b'$ should be deleted and replaced with $\sigma_{\hat{T}_0}(z)$.  For these nodes $\nu_3(z)$ is empty.
...
\begin{lem}\lemlabel{t-tree-labelling}
  Let $H$, $\rho$, $\rho'$, $Q$, and $T$ be defined as above.  Then $|\tau_{T}(v)|=h(T) + O(t(\log t + \log h(T))+\log\log m)$ for every $v\in Q$  and there exists a function $\mathds{T}:(\{0,1\}^*)^2\to \Z\cup\{\perp\}$ such that  for every $v,w\in Q$, 
  \[
...
\begin{enumerate}
  ...
  \item $y_1=y_2$.  Let $y=y_1=y_2$.  In this case (PC2)--(PC5) contains $\tau_{T_y}(v_1)$ and $\tau_{T_y}(v_2)$ (defined before \lemref{t-tree-labelling}) and we use this to test if $v_1v_2\in E(H)$.  If not, then $z_1z_2\not\in E(H\boxtimes P)$ so $z_1z_2\not\in E(G)$.  
  ...
  \item $y_2-y_1=1$: Let $y=y_1$ (so that $y_2=y+1$).  We use $\mu_y(v_1)$ and $\sigma_{T_y}(v_1)$ to compute $\sigma_{T_{y+1}}(v_1)$.  Now, $\sigma_{T_{y+1}}(v_1)$ and (PC3)--(PC4) contains $\tau_{T_{y+1}}(v_1)$ and (PC2)--(PC4) contains $\tau_{T_{y+1}}(v_2)$.  We use these to test if $v_1v_2\in E(H)$.  If not, then $z_1z_2\not\in E(H\boxtimes P)$ so $z_1z_2\not\in E(G)$.  
