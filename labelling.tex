...
Note that $|\mu_y^{\textsc{Del}}(v)| = \Oh(\log h(T_{y+1})) = \Oh(\log h(T_y))$.
  ...
  Recall also that \textsc{BulkBalance}$(\theta,k)$ calls $\textsc{Balance}(x,k)$ for each node $x$ of depth-$\theta$ in $T_y$ to restructure the subtree of $T_y$ rooted at $x$. See~\figref{rebalance-t-tree}. 
  ...
  By \lemref{all-the-obs-about-bst-and-intervals}, 
  ...
  \begin{proof}
    ...
    By \lemref{all-the-obs-about-bst-and-intervals}, $z=x_T(v)$ is the unique node $z$ in $T$ such that the path $P$ from the root to $z$ has exactly one node in $[a_w,b_w]$. 
  ...
  Since all the call $\textsc{Split}(x')$ generated by $\textsc{MultiSplit}(x_{i,1},\ldots,x_{i,c_i})$ on the subtree $T_{i}$ are called with $x'\in X$ and $[a_w,b_w]\cap X =\emptyset$, \clmref{x-switch} guarantees that 
  \begin{align*}
  x_{T'}(w) &= x_{T_y}(w), \textrm{ and}\\
  \sigma_{H^+_y,T'}(v) = \sigma_{T'}(x_{T'}(w)) &= \sigma_{T'}(x_{T_y}(w)).
  \end{align*}

  Finally, by \lemref{node-transitions} there exists a function $B'$ and a bitstring 
...
\textcolor{red}{Piotrek: I got here.}
  ...
  %and checks whether $|\sigma_{T_y}(z)| < \theta$. If this is the case then the signatures of $z$ in $T_{y}$ and $T''$ are the same, and $B$ reads the rest of $\nu_y(z)$, which corresponds to $\nu^{\textsc{Del}}_y(z)$, and 
\end{proof}
...
Recall that $S^+_y \subseteq V(H^+_y)$ and $S^+_y\subseteq V(H^+_{y+1})$ (the latter if $y\neq h$) and therefore 
...
\begin{compactenum}[(L1)]
  ...
  \item if $y\neq h$ then $\gamma(d_{T_{y+1}}(x_{T_{y+1}}(u_i)))$ for each $i\in\{1,\ldots,d\}$;
  \item $\gamma(\varphi(u_i))$ for each $i\in\{1,\ldots,d\}$;
