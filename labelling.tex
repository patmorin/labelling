...
\snote{say here that even if we don't need it now, $k$ will be $\sqrt{\log n \log \log n}$, so that the expression $O(k+k^{-1}\log n)$ (which will be seen many times), is $o(\log n)$}
