...
We define
\[
\sigma_{H,T}(v):=\sigma_T(x),\quad\text{where $x$ is a node in $x_T(C_H(v))$ of maximum depth in $T$.}
\]
...
\textcolor{red}{Piotrek: Fill in the asymptotics whenever we define labels in the proof.}
...
\begin{proof}
  ...
  Thus $P^1$ is a path from the root of $T''$ to the deepest node $x$ in $x_{T''}(C_{H^*_{y}}(v))$. 
  Similarly, let $P^2$ be the path in $T''$ encoded by $\sigma_{H'',T''}(v)$, 
  i.e.\ the path from the root to a node $x''$ that is the deepest node in $x_{T''}(C_{H''}(v))$. 
  Since $C_{H^*_{y}}(v) \subseteq C_{H''}(v)$, we conclude that $P^1$ is a subpath of $P^2$ in $T''$.
  We define 
  \[\mu_y^{\textsc{Ins}}(v) := \gamma(|\sigma_{H'',T''}(x,x'')|),\sigma_{H'',T''}(x,x'').
  \]
...
%  Let $H''$ be the subgraph of $H$ induced by $S_y\cup S_{y-1}\cup I$.
  ...
  \begin{proof}[Proof of \clmref{deletion-ancestor}]
    ...
    This can only happen if $x_{T^{\text{bef}}}(u)$ is a $T^{\text{bef}}$-ancestor of $x_i$ and $x_i$ is a $T^{\text{aft}}$-ancestor of $x_{T^{bef}}(u)$. 
    The latter is equivalent with the fact that $x_{i-1}$ is a $T^{\text{bef}}$-ancestor of $x_{T^{bef}}(u)$.
    Therefore, we have
    \[
    \sigma_{T^{\text{aft}}}(x_{T^{\text{aft}}}(u)) = \sigma_{T^{\text{aft}}}(x_i) = 
    \sigma_{T^{\text{bef}}}(x_{i-1}) \prec \sigma_{T^{\text{bef}}}(x_{T^{bef}}(u)).
    \]
...
We define 
  ...
  Assume now that $|\sigma_{H^*_{y},T_{y}}(v)| > \theta$.
