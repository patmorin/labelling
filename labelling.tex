...
\listfiles
\pdfminorversion=7 % Cyril: avoid 'pdfTeX warning: pdflatex (file ....pdf): PDF inclusion:'
\usepackage{pat}
\usepackage{paralist}
...
\author{%
\IEEEauthorblockN{Vida Dujmović}
...
% Cyril: In math mode, we should use \colon for a ponctuation colon like "f: A\to B", and use ":" for relation operator like set "\{ x : A \}" or "x := u". I tried to update all of them. If you don't like, a global replace "\colon" -> ":" should work.

A family $\mathcal{G}$ of graphs has an \emph{$f(n)$-bit adjacency labelling scheme} if there exists a function $A\colon (\{0,1\}^*)^2\to \{0,1\}$ such that for every $n$-vertex graph $G\in \mathcal{G}$ there exists $\ell\colon V(G)\to\{0,1\}^*$ such that $|\ell(v)| \le f(n)$ for each vertex $v$ of $G$ and such that, for every two vertices $v,w$ of $G$,
\[ A(\ell(v),\ell(w)) =
      \begin{cases}
        0 & \text{if $vw\not\in E(G)$;} \\
        1 & \text{if $vw\in E(G)$.}
...
\begin{lem}\lemlabel{biased-bst}
  For any finite $S\subset \R$ and any function $w\colon S\to\R^+$, there exists a binary search tree $T$ with $V(T) = S$ such that, for each $y\in S$, $d_T(y)\le\log(W/w(y))$, where $W := \sum_{y\in S} w(y)$.
\end{lem}
...
\begin{obs}\obslabel{predecessor-encoding}
  ...
  \end{enumerate}
Therefore, there exists a function $D\colon (\{0,1\}^*)^2\to\{0,1\}^*$ such that, for every binary search tree $T$ and for every two consecutive nodes $x, y$ in the sorted order of $V(T)$, there exists $\delta_T(y) \in \{0,1\}^*$ with $|\delta_T(y)|=\Oh(\log h(T))$ such that
...
\begin{lem}\lemlabel{row-code}
  There exists a function $A\colon (\{0,1\}^*)^2\to\{-1,0,1,\perp\}$ such that, for any $h\in\N$, and any $w\colon \{1,\dots,h\}\to\R^+$ there is a prefix-free code $\alpha\colon \{1,\dots,h\}\to \{0,1\}^*$ such that
  \begin{compactenum}
...
% %   so they intersect.
% %   Therefore, $\varphi(v)\neq\varphi(w)$.% since $\varphi$ is a proper colouring of $G_S$.  The second, equivalent, statement of the observation is immediate from the fact that $\sigma_T\colon V(T)\to\{0,1\}^*$ is injective, so $\sigma_T(x)$ uniquely identifies $x$.
% % \end{proof}
...
% \begin{align*}
% S_y&=\{v\in V(H)\colon (v,y)\in V(G)\}, \text{ and}\\
...
% (with mapping $f$ as in \lemref{interval-representation} and colouring $\varphi$)
, a path $P$, and a mapping of $G$ into a subgraph of $H\boxtimes P$ can be done in $\Oh(n\log n)$ time \cite{morin:fast}.  The process of computing the labels of $V(G)$ as described in \secref{pxp} and \secref{hxp} has a straightforward $\Oh(n\log n)$ time implementation.  Thus, the adjacency labels described in \thmref{main} are computable in $\Oh(n\log n)$ time for $n$-vertex planar graphs.
