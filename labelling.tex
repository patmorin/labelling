...
we define a \emph{one-phase $k$-bulk tree sequence} based on $S_0,\ldots,S_q$ to be
a sequence $T_0, \dots, T_{q}$ of binary search trees with $T_0$ an arbitrary binary search tree on node set $S_0$, and  for each $y\in \{0, \dots, q-1\}$, we have $h(T_y)>y\cdot(k+1)$ and 
