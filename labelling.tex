...
A bulk tree is parameterized by an integer $k\ge 1$.  The rounds of a bulk tree $T$ are partitioned into \emph{phases} that are kept track of using two integer values.  At the beginning of each phase, a counter $y$ is initialized to $0$ and a value $y^*$ is computed:
\begin{equation}
...
This will not be needed until the final sections, but it is helpful to keep in mind that we will ultimately take $k=\left\lceil\sqrt{\log n \log \log n}\right\rceil$, so that the expression $\Oh(k+k^{-1}\log n)$ (which appears many times), is $\omega(1)$ and $o(\log n)$.
