...
for $t$-tree $H[S^-_y \cup S^-_{y-1}]$ with an interval representation induced by $H$ and 
vertex coloring $\varphi$, and a binary search tree $T_y$,
there is a labelling $\tau_{T_y}(u)$ for any $u\in S^-_y \cup S^-_{y-1}$ such that
$|\tau_{T_y}(u)| = h(T_y) + \Oh(t\log t + t\log\log n)$ for $u\in S^-_y \cup S^-_{y-1}$, and
...
each of the edges between $(x,y)$ and $(u_i,y\pm1)$.
...
\begin{compactenum}[(L1)]
  ...
  \item $\gamma(\varphi(v))$;
  \item $\gamma(d)$;
  \item $\gamma(d_{T_{y+b}}(x_{T_{y+b}}(u_i)))$ for each $i\in\{1,\ldots,d\}$ and $b\in\{0,1\}$;
...
(L1) is of length $\log n-\log|T_y| + \log t \Oh(\log\log n)$, 
...
In total the label length is \snote{fill in}.
...
\begin{enumerate}
  ...
  we can compute $F(\tau_{T_y}(v_1),\tau_{T_y}(v_2))$ and determine if $v_1$ and $v_2$ are adjacent in $H[S^-_y]$. 
  If $v_1$ and $v_2$ are not adjacent in $H[S^-_y]$, so not adjacent in $H$, then $z_1=(v_1,y)$ and $z_2=(v_2,y)$ are not adjacent in $H\boxtimes P$ so neither in $G$.
  If $v_1$ and $v_2$ are adjacent in $H[S^-_y]$, so not adjacent in $H$, then $z_1=(v_1,y)$ and $z_2=(v_2,y)$ are adjacent in $H\boxtimes P$.  
  Now ... hmmm

\snote{Piotrek: I'm here.}

we shall look up $a(z_1)$ or $a(z_2)$
