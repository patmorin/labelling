...
a mapping $f$ assigning to every vertex $v$ of a graph $H$ an interval $f(v)$ in such a way that $f(v)$ and $f(w)$ intersect whenever $vw\in E(H)$, we call an \emph{interval representation} of $H$.
...
\textcolor{red}{Piotrek: I removed the def of $P_T(v)$. It might be that I put it back if we really need it later.}
...
all the vertices in the family clique $C_H(v)$ of $v$ lie on a single root-to-leaf path in $T$. 
Consider the path $P$ in $T$ that begins at the root of $T$ and ends at the node 
$x \in x_T(C_H(v))$ of maximum depth. 
We define $\sigma_T(v):=\sigma_T(x)$.
Again, by \lemref{all-the-obs-about-bst-and-intervals}, 
the path $P$ contains every node in $x_T(C_H(v))$. 
Note that $d_T(x_T(v))\le |\sigma_T(v)|$, and equality holds only if $x_T(v)$ is the deepest node in $x_T(C_H(v))$.


Now, we define the label $\tau_T(v)$ of a vertex $v$ in $H$. 
...
\begin{lem}\lemlabel{t-tree-labelling}
  ...
  \]
\end{lem}
Recall that the labels $\tau_T(v)$ depend on the choice of witnessing ordering or interval representation of $H$ but the function $F$ does not.
Recall also that $d_T(x_T(u)) \le |\sigma_T(u)|\leq h(T)$, for all nodes $u$ in $H$. 
...
This way the labels $\tau_T(v)$ are of length $\log m + \Oh(t\log\log m +t\log t)$.
...
Fix an interval representation of $H$ with clique number at most $\lfloor(t+1)\log_3 (2m+1)\rfloor\}$, see~\lemref{interval-representation}. 
...
%\snote{Piotrek: We have an issue with $F$ and~\lemref{t-tree-labelling}}
