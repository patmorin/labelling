...
The sets $I$ and $T$ have restrictions on them, which we now describe. For non-empty sets $X,Y\subset \R$ and an integer $a$, we say that $X$ \emph{$a$-chunks} $Y$ if, for any $a+1$-element subset $S\subseteq Y$, there exists $x\in X$, such that $\min(S)\le x\le \max(S)$. Observe that, if $X$ $a$-chunks $Y$, then $|Y|\le a(|X|+1)\le 2a|X|$.
...
\[  
  y^* := \left\lceil \frac{\log|T|}{k-\log 6}\right\rceil 
\]
...
\begin{lem}\lemlabel{bulk-insertion}
...
The following corrollary of \lemref{deletion-signature} follows immediately from the fact, for any node $z$ in a binary search tree $T$, $d_T(z)=|\sigma_T(z)|$.

\begin{cor}\corlabel{deletion-depth}
...
The following lemma is an immediate consequence of \lemref{balance-x-weight}
...
\begin{lem}
  Let $T$ be a bulk deletion tree and let $T_0,\ldots,T_{y^*}$ be the states of $T$ before the first round ($T_0$) and after each round ($T_1,\ldots,T_{y^*}$) of a phase, and let $r_0=h(T_0)-\log|T_0|$.  Then, for each $y\in\{0,\ldots,y^*\}$, $h(T_y)\le \log|T_y| + r_0 + 3y$.
\end{lem}

\begin{proof}
  We prove this lemma by establishing the following two invariants:
  ...
  Let $I_0,\ldots,I_{y^*-1}$ and $D_0,\ldots,D_{y^*-1}$ be the sets inserted and deleted so that $I_y$ and $D_y$ are the bulk insertions and bulk deletions performed in round $y$.  First, recall that $|D_y|\le (6/7)|T_y|$, which implies that $|T_{y+1}|\ge |T_y|-|D_y|\ge |T_y|/7$.  Iterating this beginning with $T_0$ implies that $|T_y|\ge |T_0|/7^y$ for each $y\in\{0,\ldots,y^*\}$. 
  
  The invariants (B1) and (B2) actually provide two upper bounds on the height of $h(T_y)$. First observe that
  \[
     |T_{0}|\left(\frac{7}{2^k}\right)^{y^*} \le 1 \enspace .
  \]
  So, by (B2) every subtree of $T_{y^*}$ of depth $y^*(k+1)$ has size at most 1.  A subtree of size 1 has height 0.  Therefore,
  \begin{align*}
    h(T_{y^*}) & \le y^*(k+1) \\
    & = \frac{(k+1)\log|T_1|}{k-\log 6 } \\
    & \le \frac{(k+1)(\log |T_{y^*}| + y^*\log 7)}{k-\log 6}  \\
...
to obtain a new subtree $T_{j}'$.  Let $T^@$ be the tree obtained by replacing each of $T_1,\ldots,T_m$ with $T_1',\ldots,T_m'$, respectively.
...
This subroutine could be implemented as a sequence of rotations \cite[Section~7.2]{morin:open} each of which reduces the height of $x$, but it will be easier to describe the end result
...
\end{lem}
 ...
 and call the resulting binary search tree $T'$.
...
\section{Transition Codes for Trees Nodes}
...
\snote{TODO: Replace $a$ with $3$ and $\log a$ with $1$.}
...
\subsection{Transition Codes for Family Cliques}
