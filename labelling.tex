...
The adjacency testing function can be implemented in time proportional to $k$ time the label length. In particular, in the case of planar graphs $t = 8$ and, if we use $k = \Oh(\sqrt{\log n / \log\log n})$, the adjacency testing procedure can be implemented in $\Oh(\log^{3/2} n\sqrt{\log\log n})$ time.  
