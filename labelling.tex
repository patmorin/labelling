...
Let $T_0,\dots,T_{q}$ be a complete one-phase $k$-bulk tree sequence with $k\ge 7$,
...
\end{compactenum}
% Cyril: According to Scheffler'90 the pathwidth of an n-vertex tree if p <= log_3(2n+1) + 1 - log_3(5). So this is a (p+1)-colouring, p+1 < log_3(2n+1) + 1, and thus with p+1 < \floor{log_3(2n+1)+1}.

% Furthermore, G: commented out as I am afraid we'll forget about this footnote \footnote{This extension is only used when we ... \textcolor{red}{Piotrek: TODO.}}, for every proper $(t+1)$-colouring $\varphi':V(H)\to\{1,\dots,t+1\}$ of $H$, there exists a colouring $\varphi'':V(H)\to\{1,\dots,\lfloor\log_3(2m+1)\rfloor\}$ such that $(\varphi',\varphi'')$ is a proper colouring of $G_S$.
\end{lem}
...
In fact, the length for (L5) is can be bounded by $\Oh(t\log\log{n})$ by ordering $u_i$'s of $C_H(v)$ according to their colors, and representing the list of the such $d$ colors differently. More precisely, in \lemref{interval-representation}, each color $\varphi(w)\in[1,\omega(G_S)]$ with $\omega(G_S)\le (t+1)\floor{\log_3(2m+1)+1}$ results of a pair $(\varphi^1(w), \varphi^2(w))$ of colors as follows: $\varphi^1(w)$ is the color of $w$ in the $(t+1)$-colouring of $H$, and $\varphi^2(w)$ is its color in the $\floor{\log_3(2m+1)+1}$-colouring of the interval graph containing the tree underlying the width-$t$ decomposition of $H$. So, the set of colors for all the vertices of the clique $C_H(v)$, including $\varphi(v)$, can be coded using an array $R$ of $t+1$ entries indexed by $\varphi^1$ colors. We set $R[\varphi^1(w)] := \varphi^2(w)$, for each $w\in C_H(v)$ (some entries may be not defined if $d<t$). Since $\varphi^1(w) \in [1,t+1]$ and $\varphi^2(w) \in [1, \floor{\log_3(2m+1)+1}]$, $\Oh(t\log\log{m}) = \Oh(t\log\log{n})$ bits suffice to described $R$. Note that thanks to $R$ and given the parameter $\omega(G_S)$, $t$ and $m$, one can check in constant time whether a vertex $w \in C_H(v)$ from its color $\varphi(w)$.

Assuming $k = o(\log{n})$, the label length is therefore $\log{n} + \Oh((k+t)\log\log{n} + k^{-1}\log{n})$. And, for a fixed constant $t$, the label length is $\log{n} + \Oh( k\log\log{n} + k^{-1}\log{n} )$.
...
The adjacency testing function is quite simple. 
% Even without using word parallelism,
This function is straightforward to implement in $\Oh(k\log n)$ time.  Note that this already allows a tradeoff between the (lower order term in the) code length and the complexity of adjacency testing.  In the case of planar graphs $t = 8$ and, if we use the shortest possible code length, $k = \Oh(\sqrt{\log n / \log\log n})$, the adjacency testing procedure can be implemented in $\Oh(\log^{3/2} n\sqrt{\log\log n})$ time. However, using $k = \Oh(\log\log{n})$ for instance, the adjacency test has time complexity $\Oh(\log{n}\log\log{n})$ while keeping a label length of $\log{n} + \Oh(\log{n}/\log\log{n})$.

%\textcolor{red}{Piotrek: TODO. paragraph on better time complexity.}

In a realistic $\log{n}$-bit word RAM model of computation with bitwise logical operations, bitwise shift operations, and a most-significant-bit\footnote{The only purpose of the most-significant-bit operation is allow decoding of the Elias $\gamma$ code in constant time.} operation the adjacency test, as described, can be implemented in $\Oh(k)$ time. So, for planar graphs this gives an adjacency test of near-constant complexity $\omega(1)$ with label length $(1+o(1))\log{n}$. The bottleneck in such an implementation is the need to evaluate the function $J$ described in \lemref{interval-transitions} and the function $B$ described in \lemref{node-transitions}.  The bottleneck in evaluating this latter function $B$ is the fact that, up to $k+1$ times, this replaces a prefix of $\sigma_T(x)$ with a string described in~\obsref{split-signature}. The end result of all of these replacements is that a prefix of $\sigma_T(x)$ is replaced with a bitstring of length $\Oh(\log n)$ that has a run-length encoding of size $\Oh(k\log\log n)$.  This is, of course, easily implemented in $\Oh(k)$ time, but perhaps a faster implementation is possible.
