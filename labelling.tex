...
\begin{lem}\lemlabel{common-ancestor}
  Let $S\subset\mathbb{I}$ be a finite set of intervals, let $X\subset\R$ be a finite set of points that stabs $S$, let $T$ be a binary search tree with $V(T):=X$, let $[a,b]\in S$, and let $x$ be the lowest common $T$-ancestor of $X\cap [a,b]$.  Then $x\in [a,b]$.
\end{lem}
...
The value of $x$ in \lemref{common-ancestor} plays an important role in our work, so we introduce a notation for it.  See \figref{x}.  For any $v=[a_v,b_v]\in\mathbb{I}$ and any binary search tree $T$ such that $V(T)$ stabs $v$, we let $x_T(v)$ denote the lowest common ancestor of $V(T)\cap v$.  By \lemref{common-ancestor} $x_T(v)\in[a_v,b_v]$.  For any subset $C\subseteq \mathbb{I}$, such that $V(T)$ stabs $C$, we define $x_T(C):=\{x_T(x):x\in C\}$.
...
We say that a binary search tree $T$ \emph{represents} some $v\in V(H)$ if $V(T)$ stabs $C_H(v)$.    .  

\begin{lem}\lemlabel{one-path}
  Let $T$ be any binary search tree that represents some $v\in V(H)$.  Then the set of nodes $x_T(C_H(v))$ are all contained a single root-to-leaf path in $T$.
\end{lem}
