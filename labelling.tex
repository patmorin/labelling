...
generalisation of planar graphs, and have been extensively studied. See
the recent bibliography on 1-planar graphs with 140 references
\cite{kobourov.liotta.ea:annotated}. The definition of $k$-planar graphs
naturally generalises for other surfaces. A graph $G$ drawn is
$(g,k)$-planar if it has a drawing in some surface of Euler genus at most
$g$ in which  each edge of $G$ is involved in at most $k$ crossings. Note
that the family of  $(g,k)$-planar  graphs is not minor closed.


\begin{thm}\thmlabel{main-all}
  For every integer $n\ge 1$,  every n-vertex graph in each of the
following families of graphs has a $(1+o(1))\log n$-bit adjacency
labelling scheme. Furthermore, for each of these families of graphs
there exists a graph $U_n$ with $n^{1+o(1)}$  vertices such that every
$n$-vertex graph in the family is isomorphic to some vertex-induced
subgraph of $U_n$.
   \begin{itemize}
   \item graphs of bounded genus and more generally apex\footnote{A
graph is \emph{apex} if it has a vertex whose removal leaves a planar
graph.}-minor free graphs.
     \item bounded degree graphs that exclude a fixed complete graph as
a minor.
...
graph.}-minor free graphs \cite{dujmovic.joret.ea:planar}.
     \item bounded degree graphs that excludes a fixed complete graph as
...
\begin{proof}
  ...
  \begin{enumerate}[($\nu_1$)]
    \item Splitting $b$ into two strings $b^-$ and $b'$ at some index $\theta$ that can be specified using $O(\log h(T_y))$ bits.
    \item Deleting a prefix of $b'$ whose length is at most $k$ and can be described using $O(\log h(T_y))$ bits.
    \item Repeatedly (up to $k+1$ times) deleting a prefix of $b'$ and replacing it with a string in the set $\Pi$ described in \obsref{split-signature}.  The length of each deleted prefix and its replacement can be described with $O(\log h(T_y))$ bits for a total of $O(k\log h(T_y))$ bits.
...
\begin{proof}
  Since $x:=x_T(v)$ is the lowest common $T$-ancestor of $X\cap[a_v,b_v]$, either $x\in X\cap[a_v,b_v]$, in which case there is nothing prove, or there is some pair $x_1,x_2\in X\cap[a_v,b_v]$ such that $x_1$ is in the subtree of $T$ rooted at the left child of $x$ and $x_2$ is in the subtree of $T$ rooted at the right child of $x$.  By the binary search tree property, $x_1<x<x_2$. But $x_1,x_2 \in [a_v,b_v]$, so $a_v\le x_1<x<x_2\le b_v$, so $x\in [a_v,b_v]$.  
\end{proof}
...
\begin{obs}\obslabel{unique-id}
    Let $T$ be a binary search tree that represents distinct $v$ and $w$ for any two distinct vertices $v,w\in V(H)$.  Then, $x_T(v)\neq x_T(w)$ or $\varphi'(v)\neq\varphi'(w)$.  Consequently, $\sigma_T(x_T(v))\neq \sigma_T(x_T(w))$ or $\varphi'(v)\neq\varphi'(w)$. 
\end{obs}
...
\begin{lem}\lemlabel{t-tree-labelling}
  Let $H$, $S$, $X$, $Q$, and $T$ be defined as above.  There exists a function $\mathds{T}:(\{0,1\}^*)^2\to \Z\cup\{\perp\}$ such that there is a prefix-free code $\tau_{H[Q]}:Q\to\{0,1\}^*$ where $|\tau_{H[Q]}(v)|=h(T) + O(t(\log t + \log h(T)))$ for every $v\in V(H)$ and, for every $v,w\in V(H)$, 
  \[
      \mathds{T}(\tau_{H[Q]}(v),\tau_{H[Q]}(w)) = \begin{cases}
      0 & \text{if $v=w$} \\
