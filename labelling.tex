...
A proof of a much more general version of \lemref{fractional} (with larger constants) is implicit in the iterated search structure of Chazelle and Guibas \cite{chazelle.guibas:fractional1}.   For the sake of completeness, \appref{fractional-proof} includes a proof of \lemref{fractional}.
...
%  Let $T'$ be the binary search tree obtained from a bulk insertion of $I$ into $T$.
  ...
  Then $|T_x|\le |T_x'|< 8|T_x|$.
\end{lem}3$-

\begin{proof}
We clearly have  $|T_x|\le |T_x'|$.  By definition, $V(T)$ 3-chunks $I:=V(T')\setminus V(T)$.  This implies that $V(T_x)$ 3-chunks $I_x:=V(T_x')\setminus V(T_x)$.  Therefore $|I_x|\le 3(|T_x|+1)$, so $|T_x'|=|T_x|+|I_x|\le 4|T_x|+3 \le 7|T_x| < 8|T_x|$.
\end{proof}

In \lemref{insertion-size} and in \lemref{deletion-size}, below, we use the constant $8$ rather than $7$ because this later simplifies calculations involving binary logarithms.
...
\begin{lem}\lemlabel{deletion-size}
  Let $T$ be any binary search tree and let $D$ be a strict subset of $V(T)$ such that $V(T)\setminus D$ $3$-chunks $D$.
  Apply $\textsc{BulkDelete}(D)$ to $T$ to obtain a new tree $T'$.
...
\begin{proof}
 We clearly have $|T'|\le |T|$.  Since $V(T)\setminus D$ $3$-chunks $D$, we have $|D|\le 3(|V(T)|-|D| + 1) \le 6(|V(T)|-|D|)$, so $|D|\le (6/7)|V(T)|$. Thus $|T'| \ge |T| - 6/7|T| = |T|/7 \geq |T|/8$.
\end{proof}
...
\begin{proof}
    ...
    For the second item, first observe that the first item implies that $x_T(v)$ has the property that $x_T(v)$ is the only node of $P_T(x_T(v))$ contained in $[a_v,b_v]$.  To see that $x_T(v)$ is the unique node with this property consider any $z\in V(T)\cap[a_v,b_v]\setminus\{x_T(v)\}$. By (the original) definition of $x_T(v)$, $z$ is in the subtree of $T$ rooted at $x_T(v)$.  Since $z\neq x_T(v)$, $P_T(z)$ contains $x_T(v)$ as an internal node so $z$ is not the only node of $P_T(z)$ contained in $[a_v,b_v]$.
\end{proof}
...
Note that this may leave some entries may be undefined if $d<t+1$, in which case we set these to $0$ to distinguish them from ``true'' colours $\{1,\ldots,t+1\}$.
...
\item Planar graphs are a subgraph-hereditary family of graphs\footnote{A family $\mathcal{G}$ of graphs is \emph{subgraph-hereditary} if, for every $G\in \mathcal{G}$ and every induced subgraph $G'\subseteq G$, $\mathcal{G}$ contains a graph isomorphic to $G'$.} and the number of $n$-vertex labelled planar graphs is $n!2^{O(n)}$ \cite{gimenez.noy:asymptotic}.  Does every subgraph-hereditary family of labelled graphs containing at most $n!2^{O(n)}$ $n$-vertex labelled graphs have a $(1+o(1))\log n$-bit adjacency labelling scheme?  This is a refinement of the \emph{implicit graph conjecture}, which asserts that any hereditary family of graphs that contains at most $2^{O(n\log n)}$ $n$-vertex graphs has an $O(\log n)$ bit adjacency labelling scheme \cite{kannan.naor.ea:implicit,spinrad:efficient}.
\end{enumerate}
