...
In this paper, which is about binary encodings, $\log x:=\log_2 x$ denotes the binary logarithm of $x$ and, for convenience, $\lg x = \log\max\{1,x\}$.  We show that there exists an \emph{adjacency labelling scheme} for planar graphs where each vertex of an $n$-vertex planar graph $G$ is assigned a $(\log n+O(\sqrt{\lg n\lg\lg n}))$-bit label and the labels of two vertices $u$ and $v$ are sufficient to determine if $uv$ is an edge of $G$.  This is optimal up to the lower order term.  An alternative, but equivalent, interpretation of this result is that, for every integer $n\ge 1$, there exists a graph $U_n$ with $n2^{O(\sqrt{\lg n\lg\lg n})}$ vertices such that every $n$-vertex planar graph is an induced subgraph of $U$.  (The correspondence between labelling schemes and universal graphs is discussed, for example, in Spinrad's book \cite[Section~2.1]{spinrad:efficient}.) 

This result is the latest in a series of results dating back to Kannan, Naor, and Rudich \cite{kannan.naor.ea:implicit0,kannan.naor.ea:implicit} who described an adjacency labelling scheme for planar graphs that uses labels of length at most $4\lceil\log n\rceil$.  Schnyder's representation of planar graphs in terms of dimension-3 posets implies a $3\lceil\log n\rceil$-bit labelling scheme for planar graphs.  Using the fact that planar graphs have arboricity 3 (the edges of a planar graph can be partitioned into 3 forests) and $log n+o(\log n)$ bit labelling schemes for trees \cite{chu, ar02, adbk17} also implies a $(3\log n + o(\log n))$-bit labelling scheme for planar graphs.

A further improvement, also based on the idea of partitioning edges into simpler graphs was obtained by Gavoille and Labourel \cite{gavoille.labourel:smaller}.  They describe a $(\log n+o(\log n))$-bit labelling scheme for bounded treewidth graphs and then partition the edges of a planar graph into two bounded treewidth subgraphs.  This results in a $(2\log n+o(\log n))$-bit labelling scheme.

Very recently, Bonamy, Gavoille, and Pilipczuk \cite{bonamy.gavoille.ea:shorter} described a $(\tfrac43\log n + o(\log n))$-bit labelling scheme for planar graphs based on a recent graph product structure theorem of Dujmović \etal\ \cite{dujmovic.joret.ea:planar}.  This product structure theorem states that any planar graph is a subgraph of the strong product $H\boxtimes P$ where $H$ is a bounded-treewidth graph and $P$ is a path.  It is helpful to think of $H\boxtimes P$ as a graph with $|V(P)|$ \emph{rows} $H_1,\ldots,H_{|P|}$, each of which is a copy of $H$ and with vertical and diagonal edges joining corresponding and adjacent vertices between consecutive rows.  We now quickly sketch the construction of Bonamy, Gavoille, and Pilipczuk \cite{bonamy.gavoille.ea:shorter}.  In the following two paragraphs we omit $o(\log n)$ terms.

The product structure theorem quickly leads to a $(\log xy)$-bit labelling scheme where $x=|V(H)|$ and $y=|V(P)|$ by using a labelling scheme for $H$ (a bounded treewidth graph) and a labelling scheme for $P$ (a path).  In a nutshell, Bonamy, Gavoille, and Pilipczuk's labelling scheme cuts $P$ (and hence $G$) into subpaths of length $n^{1/3}$ by removing $O(n^{2/3})$ vertices of $G$ that have a neighbourhood of size $O(n^{2/3})$. The resulting (cut) graph is the strong product of $H$ and  a path of length $O(n^{1/3})$ so it has a labelling scheme in which each vertex has a label of length $\log (xn^{1/3})\le (4/3)\log n$.  A slight modification of this scheme allows for the $O(n^{2/3})$ vertices adjacent to the cuts to have shorter labels, of length only $(2/3)\log n$.  The cut vertices and the vertices adjacent to them induce a bounded-treewidth graph of size $O(n^{2/3})$.  The vertices in this graph receive secondary labels of length $(2/3)\log n$.  In this way, every vertex receives a label of length at most $(4/3)\log n$.
