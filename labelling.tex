...
\begin{align}
y^*&\le \tfrac{\log |T_0|}{k-\log 7}+1\le \tfrac{\log |T_y|+y^*\log 7}{k-\log 7}+1,&&\text{(by \eqref{y-star-def}, \eqref{height-diff}, and since $y\leq y^*$)}\nonumber\\
\intertext{and thus}
...
h(T_y) & \le h(T_0) + 3y \nonumber&&\text{(by~\lemref{bulktree-old-B-properties}\itemref{lemma-item-B1})}\\
              &= \log|T_0| + 3y + r_0  \nonumber\\
...
\intertext{The second item of the lemma follows from}
    h(T_{y^*}) & \le (k+1)y^* &&\text{(by \corref{height-y-star})}\nonumber\\
    & = (k+1)\left\lceil \tfrac{\log|T_0|}{k-\log 7}\right\rceil
    \le (k+1)\left(\tfrac{\log|T_0|}{k-\log 7 } + 1\right)&&\text{(by \eqref{y-star-def})}\nonumber\\
    & = \left(\tfrac{k+1}{k-\log 7}\right)\cdot\log|T_0| + (k+1)\nonumber\\
%    & = (1+\Oh(k^{-1}))\cdot\log|T_0| + (k+1) \\
    & \le \left(\tfrac{k+1}{k-\log 7}\right)\cdot(\log|T_{y^*}| + y^*\log 7) + (k+1) && \text{(by \eqref{height-diff})}\nonumber\\
...
\begin{proof}
  ...
  \begin{proof}[Proof of \clmref{deletion-ancestor}]  
    ...
    If $x_T(v)=x_0$ then recall that $v\in S_y\subseteq S^-_y$ and $X_{y+1}\supseteq V(T_{y+1})$ contains both endpoints of each interval in $S^-_{y+1}\cup S^-_y$.  Therefore $a_v,b_v\in V(T_{y+1})$.  Therefore $r\ge 1$ and $x_1\in [a_v,b_v]$ so $x_{\tilde{T}}(v)=x_1$ and $\sigma_{\tilde{T}}(v)=\sigma_T(v)$.
