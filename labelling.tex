...
%In the remainder, for convenience we also write $d_T(v)$ as a shorthand for $d_T(x_T(v))$. 
...
\gamma(d_T(x_T(u_1))), \,\gamma(\varphi(u_1)), \,\ldots, \,
\gamma(d_T(x_T(u_d))), \,\gamma(\varphi(u_d)).
\]
\begin{lem}\lemlabel{t-tree-labelling}
  ...
  \]
\end{lem}
Note that $d_T(x_T(w)) \le |\sigma_T(w)|\leq h(T)$, for all nodes $w$ in $T$. 
