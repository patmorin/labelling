...
\item The \emph{implicit graph conjecture} asserts that any hereditary graph class\footnote{A graph class $\mathcal{G}$ is hereditary if, for every $G\in\mathcal{G}$ and every induced subgraph $G'$ of $G$, $\mathcal{G}$ contains a graph isomorphic to $G'$.} containing at most $2^{O(n\log n)}$ $n$-vertex graphs has an $O(\log n)$-bit adjacency labelling scheme \cite{X}.
% The implicit graph conjecture is known to be true for \emph{tiny} graph families containing at most $2
A variant of this conjecture, supported by \thmref{main}, would assert that any hereditary family of labelled graphs containing at most $n!2^{o(n\log n)}$ $n$-vertex labelled graphs has a $(1+o(1))\log n$-bit adjacency labelling scheme.  Proving or disproving this conjecture (or the original implicit graph conjecture) is an intriguing open problem.
\end{enumerate}
