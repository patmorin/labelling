...
\begin{proof}
  ...
  First we show that (B1) implies (i). Observe that, for any $y\{0,\ldots,y^*\}$,  
  \[  y^*\le \tfrac{\log |T_0|}{k-\log 7}+1\le \tfrac{\log |T_y|+y^*\log 7}{k-\log 7}+1 , \]
  and thus 
  \begin{equation}  
    y^*\le \tfrac{\log |T_y|}{k-2\log 7}+4 = O(k^{-1}\log|T_y|) . 
    \eqlabel{ystar-bound}
  \end{equation}
  ...
  \begin{align*}
       h(T_y) & \le h(T_0) + 3y \\
              ...
              &\le \log |T_y| + O(k^{-1}\log|T_y|) + r_0 \enspace . &\text{(by \eqref{ystar-bound})}
  \end{align*}
  ...
  \begin{align*}
    ...
    & \le (1+O(1/k))\cdot(\log|T_{y^*}| + y^*\log 7) + (k+1) & \text{(by \eqref{height-diff})}\\
    & = \log|T_{y^*}| + O(k+k^{-1}\log |T_{y^*}|) \enspace . &\text{(by \eqref{ystar-bound})}
  \end{align*}
...
\begin{proof}
  ...
  \begin{enumerate}[($\nu_1$)]
    ...
    \item Repeatedly (up to $k$ times) deleting a prefix of $b'$ and replacing it with a string in the set $\Pi$ described in \obsref{split-signature}.  The length of each deleted prefix and its replacement can be described with $O(\log h(T))$ bits for a total of $O(k\log h(T))$ bits.
...
\begin{rem}
  The $o(\log n)$ term in the label length of \thmref{pxp} is $O(k\log\log n + k^{-1}\log n)$.  An asymptotically optimal choice of $k$ is therefore $k=\lceil\sqrt{\log n\log\log n}\rceil$, yielding labels of length $\log n + O(\sqrt{\log n\log\log n})$.
\end{rem}
...
\begin{enumerate}[(PR1)]
  ...
  \item There is a function $B:(\{0,1\}^*)^2\to\{0,1\}^*$ such that, for every $x\in V(T_y)\cap V(T_{y+1})$, there is an $O(k\log\log n)$-bit string $\nu_y(x)$ such that $B(\sigma_{T_y}(x),\nu_y(x))=\sigma_{T_{y+1}(x)}$.
\end{enumerate}
...
\begin{lem}\lemlabel{interval-transitions}
  ...
  $H$, $G$, $S_1,\ldots,S_h$, $X_1,\ldots,X_h$, $T_1,\ldots,T_h$ defined as above, for each $y\in\{1,\ldots,h-1\}$ and each $v\in S_y$, there exists $\mu_y(v)\in\{0,1\}^*$ with $|\mu_y(v)|\in O(k\log\log n)$ such that $B(\sigma_{T_y}(v), \mu_y(v))=\sigma_{T_{y+1}}(v)$.
\end{lem}
...
The adjacency testing function is quite simple. Even without using word parallelism, this function is straightforward to implement in $O(tk\log n)$ time.  In the case of planar graphs $t=8$ and $k=O(\sqrt{\log n\log\log n})$, so the adjacency testing procedure can be implemented in $O(\log n\sqrt{\log n\log\log n})$ time.  Note that this already allows a tradeoff between the code length and the complexity of adjacency testing.
