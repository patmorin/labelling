...
%%-------------------------
  ...
  we have $x_{T'}(u)$ is an $T''$-ancestor of any node $x$ in $T''$ such that 
  $x\in I$ and $x\in [a_u,b_u]$.
  Therefore, $x_{T'}(u) = x_{T''}(u)$ and 
...
Since $\textsc{BulkDelete}(D)$ is a sequence of individual deletions, by a multiple applications of~\clmref{deletion-ancestor} we get that 
