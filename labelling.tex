...
\begin{lem}\lemlabel{bulktree-height-i}
  ...
  \begin{compactenum}[(i)]
    ...
    \label{item-i}
    \item $h(T_{y^*}) = \log|T_{y^*}|+\Oh(k+k^{-1}\log|T_{y^*}|)$.\label{item-ii}
  \end{compactenum}
\end{lem}
\begin{proof}
...
First we show that \ref{item-B1} implies (\ref{item-i}). Observe that, for any $y\in\{0,\ldots,y^*\}$,  we have
  \begin{align}
  ...
  \intertext{and thus}
    ...
    \eqlabel{ystar-bound}
%  \end{align}
\intertext{Therefore, for each  $y\in\{0,\ldots,y^*\}$, \ref{item-B1} gives the upper bound}
%  \begin{align*}
       h(T_y) & \le h(T_0) + 3y \nonumber\\
              &= \log|T_0| + 3y + r_0  \nonumber\\
  ...
  \end{align}
Thus, indeed \ref{item-B1} implies (\ref{item-i}).



Note that when we take a logarithm of the upper bound in \ref{item-B2} for $y=y^*$, we have
\begin{align*}
...
&=0.
\end{align*}  
Therefore, \ref{item-B2} implies that every subtree of $T_{y^*}$ of depth $y^*(k+1)$ has size at most 
$|T_0|(7/2^k)^{y^*} \geq 1$.
A subtree of size 1 has height 0.  Therefore,
  ...
  \end{align*}
Thus, indeed \ref{item-B2} implies (\ref{item-ii}).


  All that remains is to show that invariants \ref{item-B1} and \ref{item-B2} are indeed satisfied for all $y\in\{0,\ldots,y^*\}$.  The proof is by induction on $y$.  For the base case $y=0$, both properties are trivial: \ref{item-B1} asserts that $h(T_0)\le h(T_0)$ and \ref{item-B2} asserts that the subtree of $T_0$ rooted at the root of $T_0$ has size at most $|T_0|$.

  For the inductive step, assume $y\ge 1$ and invariants \ref{item-B1} and \ref{item-B2} hold for $T_{y-1}$.  First we establish invariant \ref{item-B1} as follows:
  Lemmas~\ref{lem:insertion-depth}, \ref{lem:deletion-signature}, and \ref{lem:balance-depth} imply that $h(T_y)\le h(T_{y-1})+3$.  
  By the inductive hypothesis \ref{item-B1} is satisfied for $T_{y-1}$, so $h(T_{y-1})\le h(T_0)+3(y-1)$.  Thus $h(T_y)\le h(T_{y-1}) + 3 \le h(T_0)+3y$.
  
  Next we establish \ref{item-B2}.  By \ref{item-B2} applied to $T_{y-1}$, every subtree of $T_{y-1}$ rooted at a node of depth $(y-1)(k+1)$ has size at most $|T_{0}|(7/2^k)^{y-1}$.  The first step in round $y-1$ is to execute $\textsc{BulkBalance}((y-1)(k+1))$.  By \lemref{balance-weight}, this results in a tree $T_{y-1}'$ in which every subtree rooted at a node of depth $y(k+1)$ has size at most $|T_{0}|(7/2^k)^{y-1}/2^k$.  The second step in round $y-1$ is to perform a bulk insertion on $T_{y-1}'$ to obtain a new tree $T_{y-1}''$.  
...
\begin{proof}
  ...
  \lemref{bulktree-height-i}(\ref{item-ii}) implies that $T_y$ satisfies the conditions of the lemma.  
  ...
  In this case \lemref{bulktree-height-i}(\ref{item-i}) implies that
  ...
  \begin{align*}
    r_0 &= h(T_{y_0})-\log|T_{y_0}| \\
       &= \Oh(k + k^{-1}\log|T_{y_0}|) 
        & \text{(by \lemref{bulktree-height-i}(\ref{item-ii}))}\\
