...
\begin{lem}\lemlabel{bulktree-height-i}
  ...
  \begin{compactenum}[(i)]
    \item $h(T_y)\le \log|T_y| + \Oh(k^{-1}\log|T_y|) + r_0$, for each $y\in\{0,\ldots,y^*\}$;
...
Note that when we take the logarithm of the upper bound in \itemref{item-B2} for $y=y^*$, we have
\begin{align*}
\log|T_0| + y^*\cdot\log\left(7/2^k\right) &\geq \log|T_0|+\tfrac{\log|T_0|}{\log\left(2^k/7\right)}\cdot\log\left(7/2^k\right)&&\text{(by \eqref{y-star-def})}\\
&=0.
\end{align*}  
...
A subtree of size 1 has height 0.  Therefore,
  \begin{align*}
    h(T_{y^*}) & \le (k+1)y^* \\
    & = (k+1)\left\lceil \tfrac{\log|T_0|}{k-\log 7}\right\rceil&&\text{(by \eqref{y-star-def})} \\
...
Thus, indeed \itemref{item-B2} implies (\ref{bulktree-height-item-ii}).
  ...
  For the inductive step, assume $y+1\ge 0$ and invariants \itemref{item-B1} and \itemref{item-B2} hold for $T_{y}$.  First we establish invariant \itemref{item-B1} as follows:
  Lemmas~\ref{lem:balance-depth} (rebalancing), \ref{lem:insertion-depth} (insertions), and \ref{lem:deletion-signature} (deletions) imply that $h(T_{y+1})\le h(T_{y})+3 \leq h(T_0) + 3y +3 \leq h(T_0)+3(y+1)$.  
  
  Next we establish \itemref{item-B2}.  By \itemref{item-B2} applied to $T_{y}$, every subtree of $T_{y}$ rooted at a node of depth $y(k+1)$ has size at most $|T_{0}|(7/2^k)^{y}$.  The first step in round $y$ is to execute $\textsc{BulkBalance}(y(k+1))$.  By \lemref{balance-weight}, this results in a tree $T_{y}'$ in which every subtree rooted at a node of depth $(y+1)(k+1)$ has size at most $\tfrac{|T_{0}|(7/2^k)^{y}}{2^k}$.  The second step in round $y$ is to perform a bulk insertion on $T_{y}'$ to obtain a new tree $T_{y-1}''$.  
