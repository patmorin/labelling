...
\end{compactenum}
Furthermore\footnote{This extension is only used when we ... \textcolor{red}{Piotrek: TODO.}}, for every proper $(t+1)$-colouring $\varphi':V(H)\to\{1,\ldots,t+1\}$ of $H$, there exists a colouring $\varphi'':V(H)\to\{1,\ldots,\lfloor\log_3(2m+1)\rfloor\}$ such that $(\varphi',\varphi'')$ is a proper colouring of $G_S$.
\end{lem}
...
\begin{proof}
  ...
  The tree $T_y$ is rebalanced by an application of \textsc{BulkBalance}$(\theta,k)$ with an appriopiate value of $\theta \leq h(T_y)$ and the resulting tree is $T'$.
  ...
  \]
  Consider now the node $x_{T}(w)$. Since $[a_w,b_w]\cap Z=\emptyset$ we know that $x_T(w)$ lie in one of the trees $T_i$ of the forest $T_*-Z$.
