...
Now the improved (T5) block of the labelling is an array $R$ of $t+1$ entries indexed by $\varphi'$ colours. We set $R[\varphi'(w)] := \varphi''(w)$, for each $w\in C_H(v)$. Note that some entries may be undefined if $d<t$. This way only $\Oh(t\log\log{m})$ bits suffice to encode $R$. 
