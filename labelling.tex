...
In total, the label length is $\log n + \Oh(k\log\log n+k^{-1}\log n) + \Oh(t\cdot\log\log n + t\cdot\log k + t\cdot\log t)$.
When we treat $t$ as a fixed constant we get 
...
\begin{enumerate}
  ...
  we identify the position of $v_1$ on the list of vertices in $N_H(v_2)$ (by its $\varphi$-color) 
  and then finally we lookup an appriopiate bit in $a(z_2)$ to see if the respective edge in $H\boxtimes P$ is present in $G$ or not.
  If $F(\tau_{T_y}(v_1),\tau_{T_y}(v_2))=1$, 
  we identify the position of $v_2$ on the list of vertices in $N_H(v_1)$
  and then finally we lookup an appriopiate bit in $a(z_1)$ to see if the respective edge in $H\boxtimes P$ is present in $G$ or not.
