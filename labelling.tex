...
Consider a binary search tree $T$ such that $V(T)$ stabs a family of intervals $\{f(v):v\in V(H)\}$, for some graph $H$. 
Then, we say that $T$ \emph{stabs} $H$. 
For $v \in V(H)$, we let $x_T(v)$ denote the lowest common $T$-ancestor of $V(T)\cap f(v)$, see \figref{x}.
For $V \subseteq V(H)$, we let $x_T(V):=\{x_T(v):v\in V\}$.
...
$d_T(w) \le |\sigma_T(w)|\leq h(T)$, for all $w$ in $T$. 
...
\begin{compactenum}[(L1)]
  ...
  \item $\varphi(v)$;
  \item $\gamma(d)$;
  \item $d_{T_{y+b}}(x_{T_{y+b}}(u_i))$ for each $i\in\{1,\ldots,d\}$ and $b\in\{0,1\}$;
  \item $\varphi(u_i)$ for each $i\in\{1,\ldots,d\}$;
