...
\begin{proof}
  ...
  The transition code $\mu_y(x)$ contains the value $\theta$ and enough additional information to recover $\sigma_{T_*'}(v)$ from $\sigma_{T_*}(v)$. 
  
  Now, $\textrm{balance}(x)$ identifies two special node sets $Z$ and $X$ that are built into a perfectly balanced binary search tree $T_0$ so that, eventually, $T_0$ is a subgraph of $T_*$ that contains the root of $T_*'$.   In particular, every node in $V(T_*')$ has  $T_*'$-ancestor in $Z\cup X$.
  
  For each node $v\in L_y$, let $v^*=\bigcap_{w\in C_H(v)} w$.  If $v^*\cap (Z\cup X)\neq\emptyset$ then, by definition $x_{T_*'}(w)\in Z_\cup X$ for every $w\in C_H(v)$, so $V(P_{T_*'}(v))\subseteq Z\cup X$.  Therefore, for any node such node $v$, $\sigma_{T_*'}(v)$ has length at most $h(T_0)\le k$ and can be included explicity in $\mu_y(v)$ with the addition of $O(k)$ extra bits.

  Thus, we only need to consider those $v\in L_v$ such that $v^*\cap (Z\cup X)=\emptyset$.  For each such node $v$, $P_{T_*}(v)$ has vertices in common with exactly one tree $T_{*,i}$ in the forest $T_*-Z$.  Then \textrm{balance}(x)$ calls $\textrm{multisplit}(x_1,\ldots,x_c)$ on the subtree $T_{*,i}$ with $\{x_1,\ldots,x_c\}:= X\cap V(T_{*,i})$.  This, in turn results in a number of calls to $\textrm{split}(x)$ for a node $x\in v(T_{*,i}). The following claim explains the effect of one individual call to $\textrm{split}(x)$:
  ...
  Now, consider the first call $\mathrm{split}(x_{\lceil c/2\rceil})$ on $T_{*,i}$, and let $W:=x_{T_y}(C_H(v))\cap V(T_{*,i})$.  By \clmref{x-switch}, there exists at least one $w\in W$ such that $w\neq x_{\lceil c/2\rceil}$ since, otherwise $x_{T'_y}(v)$ is a $T'_y$-ancestor of $x_{\lceil c/2\rceil}$, so $x_{T'_y}(v) \in Z\cup X$, which we have already handled above.  
  ...
  \begin{figure}
    ...
    \end{center}
    \caption{Calling $\mathrm{multipslit}(x_1,\ldots,x_c)$ on each of the depth-$\theta$ subtrees of $T_y$ except $T_*$ has no effect on $P_{T_y}(v)$.}
  ...
  % 
  % 
  %  \clmref{deletion-ancestor} implies that, for each $w\in L_y$, $\sigma_{T_y''}(x_{T_y''}(w))$ is a prefix of $\sigma_{T_y'}(x_{T_y'}(w))$.  Now, $\sigma_{T_y''}(v)=\sigma_{T_y''}(x_{T_y''}(w))$ for some $w\in C_H(v)$.  By \lemref{one-path} $\sigma_{T_{y'}}(x_{T_y'}(w))$ is a prefix of $\sigma_{T_y'}(v)$.  Using $a\preceq b$ to denote the prefix-of relation, we have
  % \[ \sigma_{T_y''}(v)=\sigma_{T_y''}(x_{T_y''}(w))
  % \preceq \sigma_{T_y'}(x_{T_y'}(w)) \preceq \sigma_{T_y'}(v) 
  % \]    
  % 
  % 
  % By definition $\sigma_{T_y''}(v)=\sigma_{T_y''}(x_{T_y''(w))$ for some $w\in C_H(v)$.  By \lemref{one-path}, $\sigma_{T_y'}(x_{T'}(w))$ is a prefix of $\sigma_{T_y'}(v)$.  By (repeated applications of) \lemref{deletion-signature}, $\sigma
  % 
