...
% Piotrek: I dont think we need Helly anywhere.
...
and an interval representation $v\mapsto[a_v,b_v]$ as in~\lemref{interval-representation}. 
...
\begin{proof}
  If $x_T(u)=x=x_T(v)$, then by \lemref{all-the-obs-about-bst-and-intervals}\itemref{itm:v-in-its-interval} intervals $[a_u,b_u]$ and $[a_v,b_v]$ each contain $x$, 
  so they intersect. 
...
\begin{enumerate}
  ...
  If $\sigma_T(x_T(w)) = \sigma_T(x_T(u_i))$ and $\varphi(w)=\varphi(u_i)$, for some $i\in\{1,\ldots,d\}$, then we conclude that $w=u_i$ (by~\obsref{unique-id}) and $F$ outputs $-1$.

  \item  Now, let $d=|C_H(w)|$ and let $u_1,\ldots, u_{d}$ be the vertices in $C_H(w)$. 
  From the label of $w$, 
  we decode the values of $d$, $\sigma_T(x_T(u_i))$ and $\varphi(u_i)$ for each $i\in\{1,\ldots,d\}$.
  If $\sigma_T(x_T(v)) = \sigma_T(x_T(u_i))$ and $\varphi(v)=\varphi(u_i)$, for some $i\in\{1,\ldots,d\}$, then we conclude that $v=u_i$ (by~\obsref{unique-id}) and $F$ outputs $1$.
...
an interval representation of $H$ with clique number at most $\lfloor(t+1)\log_3 (2m+1)\rfloor\}$, see~\lemref{interval-representation}. 
...
%\end{align*}
Note that $\sum_{y=1}^h |S_y| =n$.
%and $|S_y^-| \leq (t+1)|S_y|$. 
%Thus $\sum_{y=1}^h |S_y^-| \leq (t+1)n$. 
%Let $S^-_0=\emptyset$. 
Let $S_0=\emptyset$. 
For each $y\in\{1,\ldots,h\}$, let $X_y\subset\R$ be the set of all endpoints of intervals in $S_y\cup S_{y-1}$. 
...
For each $y\in\{1,\ldots,h\}$, let $H^*_y$ be the subgraph of $H$ induced by $S_y\cup S_{y-1}$. 
Thus each $H^*_y$ is a $t$-tree. 
...
\[
\textstyle\sum_{y=1}^h |T_y| \le 2\textstyle\sum_{y=1}^h |X_y| \le 4\textstyle\sum_{y=1}^h |S_y\cup S_{y-1}| \le 8n.
\]
...
\begin{lem}\lemlabel{interval-transitions}
  ...
  for any $H$, $P$, $G$, $\varphi$, $S_1,\ldots,S_h$, $X_1,\ldots,X_h$, and each $k$-bulk tree sequence $T_1,\ldots,T_h$ defined as above, for each $y\in\{1,\ldots,h-1\}$ and each $v\in S_y$, there exists $\mu_y(v)\in\{0,1\}^*$ with $|\sigma_{H^*_y,T_y}(v)|+|\mu_y(v)|= \log h(T_y) + \Oh(k\log h(T_y))$ such that $J(\sigma_{H^*_y,T_y}(v), \mu_y(v))=\sigma_{H^*_{y+1},T_{y+1}}(v)$.
\end{lem}
...
\begin{proof}
  ...
  The transition code $\mu_y(v)$ is the concatenation of three parts $\mu_y^{\textsc{Bal}}(v)$, $\mu_y^{\textsc{Ins}}(v)$, and $\mu_y^{\textsc{Del}}(v)$ devoted to different steps of the transformation from $T_y$ to $T_{y+1}$. 

  First we describe $\mu_y^{\textsc{Ins}}(v)$. 
  Note that $T'$ stabs $H''$. 
  Let $H''$ be the subgraph of $H$ induced by $S_{y+1}\cup S_y\cup S_{y-1}$.
  The transition code $\mu_y^{\textsc{Ins}}(v)$ will serve to move 
  from $\sigma_{H^*_{y},T'}(v)$ to $\sigma_{H'',T''}(v)$. 
  Recall that an application of $\textsc{BulkInsert}(I)$ to $T'$ 
  results in attaching some small subtrees (of height at most $2$)
  to the leaves of $T'$. 
  This way, for every $u \in S_y$, $x_{T'}(u) = x_{T''}(u)$ and therefore 
  $\sigma_{H^*_{y},T'}(u) = \sigma_{H^*_{y},T''}(u)$.
  Let $P^1$ be the path in $T''$ encoded by $\sigma_{H^*_{y},T''}(v)$. 
  Thus $P^1$ is a path from the root of $T''$ to the deepest node $x^1$ in $x_{T''}(C_{H^*_{y}}(v))$. 
  Similarly, let $P^2$ be the path in $T''$ encoded by $\sigma_{H'',T''}(v)$, 
  i.e.\ the path from the root to the node $x^2$ that is the deepest node in $x_{T''}(C_{H''}(v))$. 
  Since $C_{H^*_{y}}(v) \subseteq C_{H''}(v)$, we conclude that $P^1$ is a subpath of $P^2$ in $T''$.
  We define 
  \[\mu_y^{\textsc{Ins}}(v) := \gamma(|\sigma_{H'',T''}(x^1,x^2)|),\sigma_{H'',T''}(x^1,x^2).
  \]
  Note that $|\mu_y^{\textsc{Ins}}(v)| = \Oh(\log h(T_y)) + |\sigma_{H'',T''}(x^1,x^2)|$. 

\textcolor{red}{Piotrek: There is an issue with the assymptotics that might force us to get back and close the $S_y$ sets under taking parents.}
...
%  Let $H''$ be the subgraph of $H$ induced by $S_y\cup S_{y-1}\cup I$.
  The transition code $\mu_y^{\textsc{Del}}(v)$ serves to move 
  from $\sigma_{H'',T''}(v)$ to $\sigma_{H^*_{y+1},T''}(v)$, and then to
   $\sigma_{H^*_{y+1},T_{y+1}}(v)$. 

  Recall that $\sigma_{H'',T''}(v)$ encodes the path $P^2$ in $T''$ from the root to 
  the deepest node in $x_{T''}(C_{H''}(v))$ 
  while $\sigma_{H^*_{y+1},T''}(v)$ encodes a path, say $P^3$, in $T''$ from the root to 
  the deepest node in $x_{T''}(C_{H^*_{y+1}}(v))$.
  Since $C_{H^*_{y+1}}(v) \subseteq C_{H''}(v)$, we conclude that $P^3$ is a subpath of $P^2$ in $T''$. 
  Thus
  \[
  \sigma_{H^*_{y+1},T''}(v)\preceq \sigma_{H'',T''}(v).
  \]

  Now we need to study the changes in the signature that occur while moving from $T''$ to $T_{y+1}$. 
  ...
  \begin{proof}[Proof of \clmref{deletion-ancestor}]
    ...
    Recall that $u\in S_y\cup S_{y+1}$ so $X_{y+1}$ contains both endpoints of the interval representing $u$, say $[a_u,b_u]$. 
...
Since a bulk deletion is a sequence of individual deletions, by a multiple applications of~\clmref{deletion-ancestor} we get that 
%guarantees that $\sigma_{H^{*}_{y+1},T_{y+1}}(v)$ is a prefix of $\sigma_{H^{*}_{y+1},T''}(v)$.
  \begin{align*}
    \sigma_{H^*_{y+1},T_{y+1}}(v)
      & = \sigma_{T_{y+1}}(x_{T_{y+1}}(u)) & \text{(for some $u\in C_{H^*_{y+1}}(v)$)} \\
  ...
  \[
  \sigma_{H^*_{y+1},T_{y+1}}(v) \preceq \sigma_{H^*_{y+1},T''}(v) \preceq \sigma_{H'',T''}(v).
  \]
...
We define 
  ...
  Finally, we are going to define $\mu_y^{\text{Bal}}(v)$ that serves to move from $\sigma_{H^*_{y},T_{y}}(v)$ to $\sigma_{H^*_{y},T'}(v)$.
  The rebalancing operation that we obtain $T'$ from $T_y$ applying \textsc{BulkBalance}$(\theta,k)$ with an appriopiate value of $\theta \leq h(T_y)$.
  Recall that \textsc{BulkBalance}$(\theta,k)$ calls $\textsc{Balance}(x,k)$ for each node $x$ of depth-$\theta$ in $T_y$ to restructure the subtree of $T_y$ rooted at $x$. See~\figref{rebalance-t-tree}. 
  Let $P^1$ be the path in $T_y$ encoded by $\sigma_{H^*_{y},T_{y}}(v)$. 
  Thus $P^1$ is the path from the root of $T_y$ to the deepest node $x^1$ in $x_{T_y}(C_{H^*_{y}}(v))$.
  Clearly, if $P^1$ is not hitting vertices of depth greater than $\theta$, then $\sigma_{H^*_{y},T_{y}}(v)=\sigma_{H^*_{y},T'}(v)$.
  Thus, in the case that $|\sigma_{H^*_{y},T_{y}}(v)| \leq \theta$, we define
  \[
  \mu_y^{\text{Bal}}(v) := \gamma(\theta).
  \]

  Assume now that $|\sigma_{H^*_{y},T_{y}}(v)| > \theta$ and let $x$ be a $T_y$-ancestor of $x^1$ at depth $\theta$. 
  Recall that the application of $\textsc{Balance}(x,k)$ first indentifies two sets of nodes $Z$ and $X$ that eventually form a perfectly balanced tree $\hat{T}_0$ which forms the top part of the subtree rooted at $x$ in $T'$. 
  Clearly, whatever we also $|\sigma_{H^*_{y},T_{y}}(v)| > \theta$. 
  In the case that $\sigma_{H^*_y,T'}(v)$ encodes a path from the root of $T'$ to a node $z\in Z\cup X$, we define
  \[
  \mu_y^{\text{Bal}}(v) := \gamma(\theta),0,|\sigma_{T'}(x,z)|,\sigma_{T'}(x,z).
  \]

    \textcolor{red}{Piotrek: I got here. Under construction.}
 

  Therefore, for any $v\in S_y$, and any $\theta'\in\{0,\ldots,\theta\}$, the length-$\theta'$ prefix of $\sigma_{T_y}(v)$ and length-$\theta'$ prefix of $\sigma_{T'}(v)$ are the same.  By including the value of $\theta$ in $\mu_y(v)$, we then only need to consider the effect of the call to $\textsc{Balance}(x,k)$ where $x$ is the unique depth-$\theta$ node of $T_y$ contained in $P_{T_y}(v)$ (if any).
...
\begin{compactenum}[(L1)]
  ...
  \item $\gamma(|\sigma_{T_y}(v)|)$ and $\sigma_{T_y}(v)$; % of length $\log|T_y| + \Oh(k+k^{-1}\log n)$;
  \item $\gamma(\varphi(v))$;
  \item $\gamma(d)$;
...
(L5) is of length $\Oh(t\cdot (\log\log n + \log k))$, %\snote{Piotrek: we shall double-check}, 
...
\begin{enumerate}
  ...
  Note that (L2), (L3), (L4), (L5), (L6) contain all the pieces of the labels $\tau_{T_y}(v_1)$ and $\tau_{T_y}(v_2)$ from~\lemref{t-tree-labelling}. 
