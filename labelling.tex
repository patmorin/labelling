...
\begin{proof}
 We clearly have $|T'|\le |T|$.  Since $V(T)\setminus D$ $1$-chunks $D$, we have $|D|\le |V(T)|-|D| + 1 \le 2(|V(T)|-|D|)$ (since $D\subset V(T)$, $|V(T)|-|D|\ge 1$), so $|D|\le (2/3)|V(T)|$. Thus $|T'| \ge |T| - (2/3)|T| = |T|/3 \geq |T|/4$.
\end{proof}
...
\[
\log|T_0| - \left\lceil\tfrac{\log|T_0|}{k-2}\right\rceil\cdot(k-3)
\leq \log|T_0|-\tfrac{\log|T_0|}{k-3}\cdot(k-2)
\leq0.
\]
...
\begin{align}
q
&\le \tfrac{\log |T_0|}{k-2}+1\le \tfrac{\log |T_y|+2q}{k-2}+1,
