...
This can be done by improving the length of (T5) from $\Oh ( t\ cdot \log( t \cdot \log m ))$ to $\Oh ( t \cdot \log\log m )$. 

the length for (T5) can be bounded by $\Oh(t\log\log{n})$ by ordering the $u_i$'s in $C_H(v)$ according to their colours, and representing the list of these $d$ colours differently. More precisely, in \lemref{interval-representation}, each colour $\varphi(w)\in\{1,\dots, \omega(G_S)\}$ with $\omega(G_S)\le (t+1)\floor{\log_3(2m+1)+1}$ results of a pair $(\varphi'(w), \varphi''(w))$ of colours as follows: $\varphi'(w)$ is the colour of $w$ in the $(t+1)$-colouring of $H$, and $\varphi''(w)$ is its colour in the $\floor{\log_3(2m+1)+1}$-colouring of the interval graph containing the tree underlying the width-$t$ decomposition of $H$. So, the set of colours for all the vertices of the clique $C_H(v)$, including $\varphi(v)$, can be coded using an array $R$ of $t+1$ entries indexed by $\varphi'$ colours. We set $R[\varphi'(w)] := \varphi''(w)$, for each $w\in C_H(v)$ (some entries may be not defined if $d<t$). Since $\varphi'(w) \in \{1,\dots, t+1\}$ and $\varphi''(w) \in \{1, \dots, \floor{\log_3(2m+1)+1}\}$, $\Oh(t\log\log{m}) = \Oh(t\log\log{n})$ bits suffice to describe $R$. Note that thanks to $R$ and given the parameter $\omega(G_S)$, $t$ and $m$, one can check in constant time whether a vertex $w$ is in $C_H(v)$ using its colour $\varphi(w)$.
