...
(Note that this operation is vacuous if there is no such node.) 
...
\pnote{Piotrek: We still have some annoying references across proofs, see (1) and (2) in the proof of Lemma 15.}


A sequence $S_0,\dots,S_q$ is \emph{$3$-chunking} if $S_y$ $3$-chunks $S_{y+1}$ and $S_{y+1}$ $3$-chunks $S_y$ for each $y\in\{0,\ldots,q-1\}$.

Given an integer $k\ge 7$\footnote{$k \geq 7$ is a technical requirement, to make sure that some inequalities hold later on.} and a $3$-chunking sequence $S_0,\ldots,S_q$
with $q\leq y^*:=\left\lceil \frac{\log|S_0|}{k-3}\right\rceil$, we define
a \emph{one-phase $k$-bulk tree sequence} based on $S_0,\ldots,S_q$ to be
a sequence $T_0, \dots, T_{q}$ of binary search trees with $T_0$ an arbitrary binary search tree on node set $S_0$, and  for each $y\in \{0, \dots, q-1\}$, 
...
The sequence is \emph{complete} if $q=y^*$.
...
there exist indices $h_1,h_2,\ldots,h_{\ell}$ with $1=h_1 < h_2 < \cdots <h_{\ell} = h$  such that $T_{h_j},\ldots,T_{h_{j+1}}$ is a complete one-phase $k$-bulk tree sequence based on $S_{h_j},\ldots,S_{h_{j+1}}$
for each $j\in\{1,\ldots,\ell-2\}$, and $T_{h_{\ell-1}},\ldots,T_{h_{\ell}}$ is a (non-necessarily complete) one-phase $k$-bulk tree sequence based on $S_{h_{\ell-1}},\ldots,S_{h_{\ell}}$. 
...
Let $T_0,\ldots,T_{y^*}$ be a complete one-phase $k$-bulk tree sequence, and let $r_0=h(T_0)-\log|T_0|$.  Then,
  \begin{compactenum}[(i)]
...
Let $I_0,\ldots,I_{y^*-1}$ and $D_0,\ldots,D_{y^*-1}$ be the sets so that $T_{y+1}$ is obtained from $T_y$ by 
rebalancing and then application of $\textsc{BulkInsert}(I_y)$ and $\textsc{BulkDelete}(D_y)$, for each $y\in\{0,\ldots,y^*-1\}$.  
First, recall that by \lemref{deletion-size} we have $|T_{y+1}|\ge |T_y|/8$. 
...
\noindent Recalling once more that $y^* = \left\lceil\frac{\log|T_0|}{k-3}\right\rceil$, observe that, for each $y\in\{0,\ldots,y^*\}$,  we have
\begin{align}
y^*
&\le \tfrac{\log |T_0|}{k-3}+1\le \tfrac{\log |T_y|+3y^*}{k-3}+1,
&\text{(by \eqref{height-diff}, and since $y\leq y^*$)}\nonumber\\
...
& \le \left(\tfrac{k+1}{k-3}\right)\cdot(\log|T_{y^*}| + 3y^*) + (k+1)
&\text{(by \eqref{height-diff})}\nonumber\\
& = \log|T_{y^*}| + \tfrac{4}{k-3}\log|T_{y^*}| + 3\cdot\tfrac{k+1}{k-3}y^* + k+1\nonumber\\
...
\begin{proof}
  ...
  All that remains is to show that the conditions of the lemma are satisfied for each $y\in\{1,\ldots,h\}\setminus Y$. To show this, let $y_0=\max\{ y'\in Y: y'<y\}$.  That is, $T_{y_0}$ is the tree that began the one-phase $k$-bulk tree sequence in which $T_y$ takes part.  Let $y^*$ be defined as before with respect to the tree $T_{y_0}$, i.e.\ $y^*=\left\lceil\tfrac{\log |T_{y_0}|}{k-3}\right\rceil$.
...
\snote{Piotrek: define $\sigma_T(x,z)$}
...
\begin{proof}
  ...
  Recall also that $\gamma:\mathbb{N}\to\{0,1\}^*$ is prefix-free encoding of natural numbers such that $|\gamma(i)|=\Oh(\log i)$, for every natural number $i$ as in~\lemref{elias}.
  ...
  Recall that the scope of $\textsc{Balance}(x,k)$ is the subtree of $T_y$ rooted at $x$.
  Thus, $\sigma_{T_y}(z)$ can be affected by $\textsc{Balance}(x,k)$ only if $x$ is a $T_y$-ancestor of $z$ in $T_y$. 
  In particular when $|\sigma_{T_y}(z)| = d_{T_y}(z) \leq \theta$, the signature of $z$ does not change, that is, $\sigma_{T_y}(z)=\sigma_{T'}(z)$. In this case,  $\nu_y^{\textsc{Bal}}(z)$ is defined to be $1,\gamma(\theta)$. 
  ...
  In this case, we define $\nu_y^{\textsc{Bal}}(z)$ to be $1,\gamma(\theta),0,\gamma(|\sigma_{T'}(x,z)|),\sigma_{T'}(x,z)$.
  ...
  by~\lemref{split-height} the height of all these trees is at most $h(T_i)$;
  ...
  We define $\nu^{\textsc{Bal}}_y(z)$ in this case as follows.
    \[
  \nu^{\textsc{Bal}}_y(z) := 1,\gamma(\theta),1,\gamma(|\sigma_{\hat{T_0}}(x,z')|),\sigma_{\hat{T_0}}(x,z'),\gamma(b),
...
Let $L^-_0:=\emptyset$. 
Apply \lemref{fractional} to the sequence $L^-_{1}\cup L^-_{0},\ldots,L^-_{h}\cup L^-_{h-1}$ to obtain a $3$-chunking sequence $V_1,\ldots,V_{h}$ such that $V_y\supseteq L^-_{y}\cup L^-_{y-1}$ for each $y\in\{1,\ldots,h\}$, and $\sum_{y=1}^h |V_y|\le 2\sum_{y=1}^h |L^-_{y}\cup L^-_{y-1}|$. 
