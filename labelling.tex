...
\begin{proof}
  The proof is by induction on $h$ with the vacuous base case $h=0$.  For $h\ge 1$, choose the root of $T$ to be the unique node $y_0\in\{1,\ldots,h\}$ such that $\sum_{z=1}^{y_0-1} w(z)\le W/2$ and $\sum_{z=y_0+1}^{h} w(z)< W/2$.  Then $d_T(y_0)=0=\log 1\le \log (W/w(y_0))$.  For each node $y$ in the left subtree of $y_0$, 
  \[ 
    d_T(y) \le 1 + \log\left(\frac{\sum_{z=1}^{r-1} w(z)}{w(y)}\right) 
            \le 1 + \log \left(\frac{W/2}{w(y)}\right) 
            = \log \left(\frac{W}{w(y)}\right) , 
  \]
  and the same argument shows that $d_T(y) < \log (W/w(y))$ for each node $y$ in the right subtree of $y_0$.  
\end{proof}
