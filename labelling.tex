...
\begin{proof}
  ...
  \begin{align*}
    r_0 &= h(T_{y_0})-\log|T_{y_0}| \\
       ...
       &\le O(k + k^{-1}\log|T_{y}| + y^*) 
        & \text{(since $y-y_0 \le y^*$)} \\
       &= O(k + k^{-1}\log|T_{y}|) \enspace . & \text{(by \eqref{ystar-bound})} & \qedhere
  \end{align*}
\end{proof}
...
Given the labels of $z_1=(x_1,y_1)$ and $z_2=(x_2,y_2)$ we can test if they are adjacent as follows: Using \lemref{row-code} with $\alpha(y_1)$ and $\alpha(y_2)$ (GC1), determine which of the following applies:
\begin{enumerate}
...
In the case of planar graphs $t=8$ and, if we use the shortest possible code length, $k=O(\sqrt{\log n\log\log n})$, so the adjacency testing procedure can be implemented in $O(\log^{3/2} n\sqrt{\log\log n})$ time.  N
...
\begin{enumerate}
  \item The precise lengths of the labels in \thmref{main} is $\log n + O(\sqrt{\log n\log\log n})$.  The only known lower bound is $\log n + \Omega(1)$.  Closing the gap in the lower-order term remains an open problem.
