...
Note that $|\mu_y^{\textsc{Del}}(v)| = \Oh(\log h(T_{y+1})) = \Oh(\log h(T_y))$.
  ...
  Assume now that $|\sigma_{H^+_{y},T_{y}}(v)| > \theta$ and let $x$ be a $T_y$-ancestor of $z$ at depth $\theta$. 
  Let $Q'$ be the path in $T'$ encoded by $\sigma_{H^+_y,T'}(v)$ so $Q'$ is the path form the root of $T'$ to the deepest node $z'$ in $x_{T'}(C_{H^+_y}(v))$. 
  However \textsc{Balance}$(x,k)$ reshuffles nodes in the subtree of $x$, we will have ... hmmm
  ...
  \[
  \mu_y^{\textsc{Bal}}(v) := \gamma(\theta),0,|\sigma_{T'}(x,z)|,\sigma_{T'}(x,z).
  \]
