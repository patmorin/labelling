...
This establishes, Property~(PR4) of $T_1,\ldots,T_m$ where each code $\nu_i(y)$ has length $O(k\log\log n)$. The optimal choice of $k=\lceil\sqrt{\log n/\log\log n}\rceil$, which results each vertex of the $n$-vertex subgraph $G$ of $P\boxtimes P$ being assigned a label of length $\log n + O(\sqrt{\log n\log\log n})$.
